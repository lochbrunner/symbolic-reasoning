...
class InitialSymbolModeler(nn.Module):
    """
    Using fully connected layer
    """

    def __init__(self, initials_size, embedding_dim, rules_size):
        super(InitialSymbolModeler, self).__init__()
        self.embeddings = nn.Embedding(initials_size, embedding_dim)
...
def print_prediction(pre_symbol_to_ix, ix_to_rule, model, steps):
    with torch.no_grad():
        ...
        for step in steps:
            ...
            probs = model(symbol_tensor).exp().flatten().tolist()

            # Find tops
            tops = sorted([(v, i)
                           for i, v in enumerate(probs)], reverse=True)[:5]
...
if __name__ == "__main__":
    ...
    steps = [Step(initial=str(step.deduced),
    ...
    # Model
    model = InitialSymbolModeler(len(used_symbols), 32, len(rules))
