...
class ExecutionParameter:
    def __init__(
        ...
        training: object,
        manual_seed: bool,
        use_solved_problems: bool,
        create_fresh_model: bool,
        dont_dump_model: bool = False,
        just_dump_model: bool = False,
        **kwargs,
    ):
    ...
    def add_parsers(parser: ArgumentParser):
        parser.add_argument(
            '-d', '--device', choices=['cpu', 'cuda', 'auto'], default='cpu'
        )
    ...
    stat = [
        {
            'parameter': hp,
            'results': [
                {'error': error.as_dict(), 'epoch': epoch}
                for (epoch, error, loss) in logbook
            ],
        }
...
def main(
    ...
    learn_params: LearningParameter,
    scenario_params: ScenarioParameter,
    config,
    early_abort_hook=None,
    no_sig_handler=False,
    ...
    train_loader_params = {
        'batch_size': int(learn_params.batch_size),
        'shuffle': True,
        **common_loader_params,
    }

    validate_loader_params = {'batch_size': 8, 'shuffle': False, **common_loader_params}
    ...
    policy_weight = torch.as_tensor(
        dataset.label_weight, device=device, dtype=torch.float
    )
    value_weight = torch.as_tensor(
        dataset.value_weight, device=device, dtype=torch.float
    )
    ...
    try:
        ...
        def report(epoch, epoch_loss, **kwargs):
            ...
            model.eval()
            validation = validate(
                model, validation_dataloader, no_negative=False, **kwargs
            )
            validation_training = validate(
                model, training_dataloader, no_negative=False, **kwargs
            )
            ...
            if azure_run is not None:
                ...
                error.in_possibilities_negative.log(
                    azure_run.log, 'just possibilities (negative)'
                )
            ...
            if writer:
                ...
                error.in_possibilities_positive.log_bundled(
                    writer, 'policy/just possibilities', epoch
                )
                error.in_possibilities_negative.log_bundled(
                    writer, 'policy/just possibilities (negative)', epoch
                )

                train_error = validation_training.error
                train_error.exact.log_bundled(
                    writer, 'train-policy/exact with padding', epoch
                )
                train_error.exact_no_padding.log_bundled(
                    writer, 'train-policy/exact', epoch
                )
                train_error.in_possibilities_positive.log_bundled(
                    writer, 'train-policy/just possibilities', epoch
                )
                ...
                writer.add_scalar(
                    'loss/validation/policy', validation.policy_loss, epoch
                )
                ...
                writer.add_scalar(
                    'optimizer/learning rate', optimizer.param_groups[0]['lr'], epoch
                )
                ...
                writer.add_scalars(
                    ...
                    {
                        rule_mapping[i]: c
                        for i, c in enumerate(validation.predicted_rule_distribution)
                    },
                    epoch,
                )
        ...
        if writer:
            writer.add_hparams(
                ...
                metric_dict={
                    'kpi/value-all': float(error.value_all),
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(
        '-c', '--config-file', exclude="scenario-*", prog='deep training'
    )
    ...
    changed_parameters = grid_search.get_range_names(
        model_hyper_parameters, vars(config)
    )
    for model_hyper_parameter, arg in grid_search.unroll_many(
        model_hyper_parameters, vars(config)
    ):
        ...
        result = main(
            ...
            learn_params=LearningParameter.from_config_and_hyper(
        ...
        stats.append(
            (
                grid_search.strip_keys(
                    model_hyper_parameter, arg, names=changed_parameters
                ),
                result,
            )
        )
