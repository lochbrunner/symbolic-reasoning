...
def make_namespace(value):
    if isinstance(value, dict):
        return Namespace(
            **{k.replace('-', '_'): make_namespace(v) for k, v in value.items()}
        )
...
def setup_logging(verbose, log, once: bool, **kwargs):
    ...
    logging.basicConfig(
        level=logging.getLevelName(loglevel), format=log_format, datefmt='%I:%M:%S'
    )
    if once:
...
def split_dataset(dataset: torch.utils.data.Dataset, validation_ratio=0.1):
    ...
    return torch.utils.data.random_split(
        dataset, [trainings_size, validation_size], generator=generator
    )
