...
from solver.inferencer import TorchInferencer
...
def main(options, config, early_abort_hook=None):
    ...
    try:
        ...
        for i, rule in enumerate(scenario.rules, 1):
            ...
            logger.debug(
                f'Using rule {i:2}# {rule.name.ljust(max_width)} {rule.verbose}'
            )
        ...
        for scenario_rule in scenario.rules:
            if str(scenario_rule) not in used_rules:
                logger.warning(
                    f'The rule "{scenario_rule}" was not in the model created by the training.'
                )
        ...
        optimizer = optim.Adadelta(
            inferencer.model.parameters(), lr=learn_params.learning_rate
        )
        ...
        for iteration in range(config.evaluation.problems.iterations):
            ...
            if success_rate.correct == 0.0:
                ...
                return
            logger.info(
                f'Solved: {success_rate.verbose} with {needed_fits.statistic} fits in iteration {iteration}'
            )
            ...
            train(
                ...
                policy_weight=dataset.label_weight,
                value_weight=dataset.value_weight,
            )
    ...
    with tqdm(total=100, desc='Create index', leave=False) as progress_bar:
        intro.create_index(
            lambda progress: progress_bar.update(min(100 * progress, 100))
        )
